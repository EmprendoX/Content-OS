      ...
      <Tabs defaultValue="variants">
        ...
        <TabsContent value="runs" className="mt-4">
          <div className="space-y-2">
            {runs.map((run) => (
              <div key={run.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border px-3 py-2 text-sm">
                ...
                <div className="text-xs text-muted-foreground">{fmtDateTime(run.createdAt)}</div>
                {run.error && <div className="w-full text-xs text-red-700">{run.error}</div>}
              </div>
            ))}
          </div>
        </TabsContent>
